@if ($type == 'ajax-edit')
    <button data-route="{{ $route }}" data-value="{{ $row->id }}" class='btn btn-primary btn-sm mb-2'
        onclick="ajaxEdit(this)" data-bs-placement="top" data-bs-toggle="tooltip" data-bs-original-title="@lang('app.edit')"
        title='@lang('app.edit')'><i class='fa fa-edit'></i></button>
@endif

@if ($type == 'edit')
    <a href="{{ route($route . '.edit', $row->id) }}" class='btn btn-primary btn-sm mb-2' data-bs-placement="top"
        data-bs-toggle="tooltip" data-bs-original-title="@lang('app.edit')" title='@lang('app.edit')'><i
            class='fa fa-edit'></i> </a>
@endif

@if ($type == 'delete')
    <button data-route="{{ route($route . '.destroy', $row->id) }}" class='_delete btn btn-sm btn-danger mb-2'
        data-bs-placement="top" data-bs-toggle="tooltip" data-bs-original-title="@lang('app.cannot-undo')"
        title='@lang('app.delete')'><i class='fa fa-trash'></i></button>
@endif
@if ($type == 'ajax-delete')
    <button data-route="{{ $route }}" data-value="{{ $row->id }}"
        onclick="ajaxDelete(this, '{{ $src }}')" class='btn-sm btn btn-danger mb-2' data-bs-placement="top"
        data-bs-toggle="tooltip" data-bs-original-title="@lang('app.cannot-undo')" title='@lang('app.delete')'><i
            class='fa fa-trash' style="vertical-align: middle;"></i></button>
@endif
@if ($type == 'view')
    <a href="{{ route($route . '.show', $row->id) }}" class=' btn btn-sm btn-secondary mb-2' data-bs-placement="top"
        data-bs-toggle="tooltip" data-bs-original-title="@lang('app.show-details')" title="@lang('app.show-details')"><i
            class='fa fa-eye'></i></a>
@endif
